...
// bodyParser 모듈이 있는데 모듈을 설치하고 => 현재는 express 자체적으로 포함
// body에 데이터를 가져온다 
app.use(express.urlencoded({ extended: false }));  // 미들웨어
// app 객체 만들고, 다른 url 처리 전에만 호출되면 된다
...
// post의 경우는    app.use(express.urlencoded({ extended: false })); 가 선행되고 나면 request.body.x로 처리한다
